#include "./include/preprocessing/thinning.hpp"
...
cv::Mat fingervein::Thinning::invertColor(const cv::Mat &img) {
    cv::Mat imgInverted;
...
template<class _T>
cv::Rect fingervein::Thinning::boundingBox(const cv::Mat_<_T> &img) {
...
cv::Rect fingervein::Thinning::copy_bounding_box_plusone(const cv::Mat1b &img, cv::Mat1b &out, bool crop_img_before) {
...
bool fingervein::Thinning::thinGuoHallFast(const cv::Mat1b &img, bool inverted, bool crop_img_before, int max_iters) {
    if (inverted) {
        ...
        return this->thin_fast_custom_voronoi_fn(this->invertColor(img), inverted, need_set_guo_hall, crop_img_before,
                                                 max_iters);
    }
}

bool fingervein::Thinning::thin_fast_custom_voronoi_fn(const cv::Mat1b &img, bool inverted, VoronoiFn voronoi_fn,
                                                       bool crop_img_before, int max_iters) {
    if (inverted) {
    ...
    while (change_made && niters < max_iters) {
        ...
        for (unsigned short iter = 0; iter < 2; ++iter) {
            ...
            size_t rows_to_set_size = rows_to_set.size();
...
cv::Mat fingervein::Thinning::getImgSkeleton() const noexcept {
    return imgSkeleton;
}

cv::Mat fingervein::Thinning::getImgSkeletonInverted() const noexcept {
